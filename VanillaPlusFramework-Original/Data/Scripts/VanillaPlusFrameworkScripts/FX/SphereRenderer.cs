...
namespace VanillaPlusFramework.FX
{
    public class SphereRenderer : IRenderObject
    {
        ...
        public bool Update()
        {
            ...
            if (startTime > 0)
            {
                Thickness = MathHelper.Lerp(StartThickness, 0f, ((float)startTime - (float)time) / (float)startTime);
                Color.W = MathHelper.Lerp(StartColor.W, 0f, ((float)startTime - (float)time) / (float)startTime);
            }
